...
const VendasTab: React.FC = () => {
  ...
  const [selectedVendaForNumbers, setSelectedVendaForNumbers] = useState<{ id: string; numeros: number[] } | null>(null);
  ...
  return (
    <div className="animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select 
              ...
              onValueChange={(value: 'todos' | 'pendente' | 'concluida') => setFiltrosVendas({ ...filtrosVendas, status: value })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Select 
              ...
              onValueChange={(value: 'todos' | 'hoje' | 'semana' | 'mes') => setFiltrosVendas({ ...filtrosVendas, periodo: value })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
        ...
        <table className="w-full">
          ...
          <tbody>
            {vendasFiltradas.map((venda) => {
              const numeros = venda.numeros_cartelas.split(',').map((num) => parseInt(num.trim(), 10)).filter(Number.isFinite);
              const numerosPreview = numeros.slice(0, 4);
              const hasMoreNumbers = numeros.length > 4;
                ...
                <tr key={venda.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="flex flex-wrap gap-1.5 max-w-xs">
                      {numerosPreview.map((num, idx) => (
                        <span
                          key={`${venda.id}-${num}-${idx}`}
                      ...
                      {hasMoreNumbers && (
                        <Button
                          ...
                          onClick={() => setSelectedVendaForNumbers({ id: venda.id, numeros })}
                        >
                          Ver mais
                        </Button>
                      )}
      ...
      <Dialog open={!!selectedVendaForNumbers} onOpenChange={(open) => { if (!open) setSelectedVendaForNumbers(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Números Vendidos</DialogTitle>
          </DialogHeader>
          <div className="flex flex-wrap gap-2 max-h-[60vh] overflow-y-auto pr-1">
            {selectedVendaForNumbers?.numeros.map((num, idx) => (
              <span
                key={`modal-${selectedVendaForNumbers.id}-${num}-${idx}`}
                className="inline-flex items-center justify-center min-w-[3rem] px-3 py-1.5 bg-emerald-500 text-white rounded-md text-sm font-bold shadow-sm"
              >
                {formatarNumeroCartela(num)}
              </span>
            ))}
